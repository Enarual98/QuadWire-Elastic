...
def generate_fake_data(nElems, directory_path=r".\thermal_data\fake_data", cooling=10, Tbuild=353.15, Tsubstrate=323.15, Tamb=293.15):
    """
    Function generating fake temperature data assuming material is deposited at temperature Tbuild and cools down to room temperature Tamb linearly in cooling steps without ever heating again
    ...
    if os.path.exists(directory_path):
        ...
        files_count = sum(1 for _ in os.listdir(directory_path) if _.startswith('temp_') and _.endswith('.txt'))

        # If the directory contains more than nNodes files, remove files, keeping only nNodes files to be overwritten
        if files_count > nElems:
            files_in_directory = sorted(os.listdir(directory_path))  # Sort files by name
            for file_name in files_in_directory[nElems:]:
                file_path = os.path.join(directory_path, file_name)
                os.remove(file_path)
            print(f"Previous files have been removed from {directory_path}.")

        # If the directory already contains exactly nNodes files, return
        elif files_count == nElems:
            print("Thermal data not modified.")
            return

    # Generate files only if the directory doesn't exist or doesn't contain nNodes files
    else:
        os.makedirs(directory_path)  # Create directory if it doesn't exist

    # Generate temperatures for each file
    temperatures = Tamb * np.ones(nElems)
    temperatures[-cooling:] = np.linspace(Tamb, Tbuild, cooling)[-min(nElems,cooling):]
    for i in range(1, nElems + 1):
        file_name = f"temp_{str(i).zfill(4)}.txt"
        file_path = os.path.join(directory_path, file_name)

        with open(file_path, 'w') as file:
            file.write('\n'.join(map(str, temperatures[-i:])))
    print(f"Temperatures have been generated and stored in {directory_path}.")


generate_fake_data(21)
