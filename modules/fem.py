...
def alpha2beta_matrices(Hn, Hb, nNodesTot, nUncNodes):
    ...
    Tc = sp.sparse.kron(Tv.T, sp.sparse.eye(nNodesTot))
    # with all uncoupled Nodes
    Ta = sp.sparse.kron(Tv.T, sp.sparse.eye(nUncNodes)) #nUncNodes
...
def local2global_matrices(Xunc, uncElems, elemOrder):
    ...
    P = np.hstack((t, n, b))  # all frames (nNodes,3*3)
    P = np.tile(P, (4, 1))  # replicate for {TL,TR,BL,BR} (4*nNodes,3*3)
    P = np.split(P, 9, 1)  # ((4*nNodes),3*3)
    P = [sp.sparse.diags(p.flatten()) for p in P]  # ((4*nNodes,4*nNodes),3*3)
    P = sp.sparse.bmat(np.array(P).reshape((3, 3)).T)  # (12*nNodes,12*nNodes)
...
def elem2node(nNodes, nLayers_h, nLayers_v):
    ...
    elem2node = sp.sparse.kron(sp.eye(nLayers), elem2node)
    return elem2node
