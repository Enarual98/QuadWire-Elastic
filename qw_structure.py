...
def structure(L, Hn, Hb, beadType, layerType, stacking_offset, zigzag, nNodes, nLayers_h, nLayers_v, dT, loadType, elemOrder, quadOrder, optimizedBehavior, toPlot, clrmap="stt", scfplot=10):
    """
    ...
    zigzag : bool
        back and forth trajectory if True
    nNodes : int
    ...
    # undeformed shape
    x = X[:, :, np.newaxis] + 0.5 * (Hn * nm[:, :, np.newaxis] * np.array([[[-1, 1, -1, 1]]]) + Hb * bm[:, :, np.newaxis] * np.array([[[1, 1, -1, -1]]]))
    ...
    if toPlot:
        ...
        if clrmap == "stt":
            clr = sigmatt_elem[:, 0]  # sigmatt = sp.sparse.linalg.lsqr(N @ Sni, Sigma[0:nQP])[0][:, None]  # clr = sigmatt[:, None] * [[1, 1, 1, 1]]
        elif clrmap == "nrg":
            clr = nrg
        # clr = sigmatt_elem[:,0]
    ...
    return U, Eps, Sigma, nrg, qp2elem, nQP, x, Elems
